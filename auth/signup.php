...
// Initialize variables for sticky values to avoid undefined notices on first load
$photo_name = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $mailing_address = trim($_POST['mailing_address'] ?? ''); // ✅ NEW
    $children = isset($_POST['children']) ? $_POST['children'] : [];

    // ✅ Use NULL if phone/address is empty
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (!$error) {
                    try {
                        ...
                        // ✅ Create member (now includes mailing_address)
                        $stmt = $pdo->prepare("
                            INSERT INTO members (user_id, family_photo, spouse_name, spouse_phone, spouse_email, mailing_address)
                            VALUES (?, ?, ?, ?, ?, ?)
                        ");
                        $stmt->execute([
                            $user_id,
                        ...
                        // Clear sticky fields on success
                        $name = $email = $phone = $password = $confirm_password = '';
                        $spouse_name = $spouse_phone = $spouse_email = '';
                        $mailing_address = '';
                        $children = [];
                ...
                <form method="POST" enctype="multipart/form-data" novalidate>
                    <div class="mb-3">
                        ...
                        <?php if (!empty($photo_name) && $photo_name != 'default.png'): ?>
                            <p class="text-muted">Photo already uploaded: <?php echo htmlspecialchars($photo_name, ENT_QUOTES, 'UTF-8'); ?></p>
                        <?php endif; ?>
                    </div>
                    ...
                    <!-- ✅ NEW: Single-line Mailing Address (optional) -->
                    <div class="mb-3">
                        <label>Mailing Address</label>
                        <input type="text" name="mailing_address" class="form-control" placeholder="Street, City, Province/State, Postal/ZIP, Country"
                               value="<?php echo htmlspecialchars($mailing_address ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        <!-- If you want to make it multiline, swap to a textarea:
                        <textarea name="mailing_address" class="form-control" rows="2"><?php echo htmlspecialchars($mailing_address ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
                        -->
                    </div>
